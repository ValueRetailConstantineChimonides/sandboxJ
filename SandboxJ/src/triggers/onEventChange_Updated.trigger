...
*/
trigger onEventChange_Updated on Event (after insert, after update) {
            
    /* B2B Alerts */
    if (Trigger.isDelete) {
        B2BContactReportAlertGenerator_updated.cleanOldRecipients(Trigger.oldMap.keySet(), null);
        return;
    }
    ...
    if (!b2bAlertEventMapToSend.isEmpty()) {
        ...
        B2BContactReportAlertGenerator_updated.cleanOldRecipients(b2bAlertEventMapToSend.keySet(), null);
        //generate new recipient holders
        B2BContactReportAlertGenerator_updated generator = new B2BContactReportAlertGenerator_updated();
        generator.generateRecipients(b2bAlertEventMapToSend.values());
        
        //delete old Recipient Holders
        B2BContactReportAlertGenerator_updated.cleanOldRecipients(null, 7);
    }
    
    if (!b2bAlertCancelledEventId.isEmpty()) {
        //Send_Email_Alert__c used to be true but user changed it to False
        B2BContactReportAlertGenerator_updated.cleanOldRecipients(b2bAlertCancelledEventId, null);  
    }
